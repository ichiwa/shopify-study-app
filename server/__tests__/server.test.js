...
describe('shopify-app-node server', async () => {
  ...
  test.concurrent(
    ...
    async () => {
      ...
      const response = await request(productionApp).get('/something').set('Accept', 'text/html');
  ...
  test('redirects to auth if the app needs to be [re]installed', async () => {
    const response = await request(app).get('/?shop=test-shop').set('Accept', 'text/html');
  ...
  describe('content-security-policy', () => {
    test('sets Content Security Policy for embedded apps', async () => {
      ...
      const response = await request(app).get('/?shop=test-shop.myshopify.test');
    ...
    test('sets header correctly when shop is missing', async () => {
      ...
      expect(response.headers['content-security-policy']).toEqual(`frame-ancestors 'none';`);
    });
  ...
  test('renders toplevel auth page', async () => {
    ...
    const response = await request(app).get(`/auth/toplevel?shop=test-shop&host=${host}`).set('Accept', 'text/html');
  ...
  describe('handles the callback correctly', () => {
    test('redirects to / with the shop and host if nothing goes wrong', async () => {
      const validateAuthCallback = vi.spyOn(Shopify.Auth, 'validateAuthCallback').mockImplementationOnce(() => ({
        shop: 'test-shop',
      ...
      const response = await request(app).get('/auth/callback?host=test-shop-host&shop=test-shop');

      expect(validateAuthCallback).toHaveBeenLastCalledWith(expect.anything(), expect.anything(), {
        host: 'test-shop-host',
        shop: 'test-shop',
      });
      ...
      expect(response.status).toEqual(302);
      expect(response.headers.location).toEqual('/?shop=test-shop&host=test-shop-host');
    });
    ...
    test('returns 400 if oauth is invalid', async () => {
      ...
      const response = await request(app).get('/auth/callback?host=test-shop-host');
    ...
    test('processes webhooks', async () => {
      ...
      const response = await request(app)
        ...
        .set(
          ...
          createHmac('sha256', Shopify.Context.API_SECRET_KEY).update('{}', 'utf8').digest('base64')
        )
