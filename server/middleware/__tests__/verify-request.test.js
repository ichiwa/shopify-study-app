...
describe('verify-request middleware', async () => {
  ...
  describe('with a Session & returnHeader enabled', async () => {
    ...
    test('active & valid session does not redirect to /auth', async () => {
      const session = new Shopify.Session.Session('1', 'test-shop', 'state', app.get('use-online-tokens'));
      session.scope = Shopify.Context.SCOPES;
      session.accessToken = 'access-token';
      session.expires = null;
      vi.spyOn(Shopify.Utils, 'loadCurrentSession').mockImplementationOnce(() => session);
    ...
    test('inactive session with returnHeader returns 403 with the right headers', async () => {
      ...
      expect(response.headers['x-shopify-api-request-failure-reauthorize']).toBe('1');
      expect(response.headers['x-shopify-api-request-failure-reauthorize-url']).toBe(`/auth?shop=test-shop`);
    });
  ...
  test('redirects when the session and shop are mismatched', async () => {
    const session = new Shopify.Session.Session('2', 'test-shop2', 'state', app.get('use-online-tokens'));
    vi.spyOn(Shopify.Utils, 'loadCurrentSession').mockImplementationOnce(() => session);
